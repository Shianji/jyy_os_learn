//用户态loader的实现，通过按照System V的ABI规范读取可执行文件的ELF文件内容实现
#include <stdint.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
...
#define push(sp, T, ...) ({ *((T*)sp) = (T)__VA_ARGS__; sp = (void *)((uintptr_t)(sp) + sizeof(T)); })//这里压栈的方式是从下至上，所以后面再压入参数数量、参数、环境变量时的顺序也与ABI规定的顺序相反
